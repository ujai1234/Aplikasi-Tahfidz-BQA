  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      try {
        ...
        // BFS Flood Fill from corners to remove white outer background
    ...
    <div className={`relative flex items-center justify-center shrink-0 ${sizeClasses} ${className}`}>
      {processedSrc ? (
        ...
        <img
          src={rawSrc}
          alt={alt}
          className={`w-full h-full object-contain mix-blend-multiply filter brightness-105 saturate-110 drop-shadow-[0_0_2px_rgba(255,255,255,0.95)] drop-shadow-[0_2px_4px_rgba(0,0,0,0.25)] ${imgClassName}`}
          onError={() => {
